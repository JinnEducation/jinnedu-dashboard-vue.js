...
<template>
  <toolbar title="Conference Upload File" />
  <upload-file-modal
    ref="uploadFileModal"
        ...
        <div class="card-header border-0 pt-6">
          <div class="card-title">
            <div class="d-flex align-items-center position-relative my-1">
              ...
              <label for="search-files" class="sr-only">{{ t("global.search-files") }}</label>
              <input
                id="search-files"
                type="text"
                name="search-files"
                :placeholder="t('global.search-files')"
                data-kt-files-table-filter="search"
                class="form-control form-control-solid w-250px ps-14"
                @keyup.enter="searchDataTableBodyRows" />
            </div>
          </div>
          <div class="card-toolbar">
            <div data-kt-file-table-toolbar="base" class="d-flex justify-content-end">
              <button type="button" class="btn btn-primary" @click="showAddModal">
                ...
                {{ t("global.add-button") }} {{ t("global.file") }}
              </button>
            </div>
          </div>
        </div>
        ...
        <template v-else>
          <div class="card-body py-4">
            <data-table
              ...
              <template #name="{row: file}">
                {{ file.name }}
              </template>
              <template #actions="{row: file}">
                <a
                  :href="`${SERVER_PATH}/${file.path}`"
...
import UploadFileModal from "@/components/admin/modals/forms/upload-file-modal.vue"
...
export default defineComponent({
  ...
  components: {Toolbar, UploadFileModal, DataTable, InformationCard},
  setup() {
    ...
    const getDataTableBodyRows = function getDataTableBodyRows(queryString = "") {
      loading.value = true
      axiosClient
        .get(`/conferences/my-files/${conference.value}${queryString}`)
        .then((response) => {
    ...
    const deleteFile = function deleteFile(id) {
      Swal.fire({
        icon: "error",
        ...
        if (result.isConfirmed) {
          axiosClient.delete(`/conferences/upload-file/delete/${id}`).then(() => {
    ...
    const deleteFewFiles = function deleteFewFiles() {
    ...
    const showAddModal = function showAddModal() {
      ...
      showModal(uploadFileModal.value.uploadFileModal.modal)
    }

    const showUpdateModal = function showUpdateModal(id) {
      idCurrent.value = conference.value
      axiosClient
        .get(`/conferences/upload-file/${conference.value}/show/${id}`)
        .then((response) => {
