...
export default defineComponent({
  ...
  setup() {
    ...
    const handleSuccessfulSubmission = function handleSuccessfulSubmission() {
    ...
    const handleSubmissionError = function handleSubmissionError(errorMessage) {
    ...
    const resetFormButton = function resetFormButton() {
      // Reset form button attributes after submission
      ...
      validation[stepperInstance.getCurrentStepIndex() - 1].validate().then((status) => {
        if (status.toString().toLowerCase() === "valid") {
          ...
          store
            ...
            .then((response) => {
              console.log(response, data.value.accountType)
                ...
                axiosClient
                  .post("/tutor/about", formDataPersonal, {
                    headers: {
                      ...
                      "Content-Type": "multipart/form-data" // Assuming formDataPersonal is FormData
                    }
                  })
                  .then(() => {
                    ...
                    axiosClient
                      ...
                      .then(() => {
                        ...
                        let availabilitiesPromise = null
                        availabilityArray.forEach((element) => {
                          ...
                          if (element.hour_from && element.hour_to) {
                            ...
                            availabilitiesPromise = axiosClient.post(
      ...
      if (isProfile) {
        axiosClient
          ...
          .then((profileResponse) => {
            ...
            // Check if user.abouts is not empty
              ...
              Object.keys(tutorAvailabilities).forEach((tutorAvailability) => {
                user.availabilities.forEach((userAvailability) => {
                  ...
                  if (tutorAvailabilities[tutorAvailability].id === userAvailability.day?.id) {
                    console.log("Pushing", userAvailability.hour_from, userAvailability.hour_to)
        ...
        if (validator) {
          validator.validate().then((status) => {
            if (status.toLowerCase() === "valid") {
              // let isEmailExists = false // Variable to track email existence
              ...
              if (stepper.getCurrentStepIndex() === 2) {
                ...
                if (isProfile) {
                  axiosClient
                    ...
                    .then((profileResponse) => {
                      console.log("test")
                      user = profileResponse.data.result
                      const formData = new FormData()
                      formData.append("email", data.value.email.toLowerCase())
                      // Check if user is defined before appending user.id to formData
                      if (user && user.id) {
                        ...
                        .then((response) => {
                          if (response.data.success) {
                            ...
                            countriesPromise = axiosClient
                              .get("/front/constants/countries?limit=500")
                              .then((countryResponse) => {
                                countries.value = countryResponse.data.result.data
                              })

                            languagesPromise = axiosClient
                              .get("/front/constants/languages?limit=500")
                              .then((languageResponse) => {
                                languages.value = languageResponse.data.result.data
                              })

                            subjectsPromise = axiosClient
                              .get("/front/constants/subjects")
                              .then((subjectResponse) => {
                                subjects.value = subjectResponse.data.result.data
                              })

                            experiencesPromise = axiosClient
                              .get("/front/constants/experiences")
                              .then((experienceResponse) => {
                                experiences.value = experienceResponse.data.result.data
                              })

                            situationsPromise = axiosClient
                              .get("/front/constants/situations")
                              .then((situationResponse) => {
                                situations.value = situationResponse.data.result.data
                              })

                            specializationsPromise = axiosClient
                              .get("/front/constants/specializations")
                              .then((specializationResponse) => {
                                specializations.value = specializationResponse.data.result.data
                              })
                            ...
                            Promise.allSettled([
                              ...
                              if (user.type !== 2) {
                                ...
                                profileFormData.append(
                                  "avatar",
                                  avatarSelected.value ? avatarSelected.value : ""
                                )
                                profileFormData.append("first_name", data.value.firstName)
                                profileFormData.append("last_name", data.value.lastName)
                                profileFormData.append("email", data.value.email.toLowerCase())
                                axiosClient
                                  .post("/update-profile", profileFormData)
                                  .then((response) => {
