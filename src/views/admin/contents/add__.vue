        ...
        <el-form
            ...
            <div v-if="content !== 'links'" class="card card-flush py-4">
              <div class="card-header">
                <div class="card-title">
                  <h2 class="required">{{ t("global.Attachment") }}</h2>
                </div>
              </div>
          ...
          <div class="d-flex flex-column flex-row-fluid gap-7 gap-lg-10">
              ...
              <div class="card-body pt-0">
                <div class="mb-10 fv-row">
                  <label :for="`${content}-title`" class="required form-label">
                    {{ t(`global.${content}`) }} {{ t("global.Title") }}
                  </label>
                ...
                <div v-else class="fv-row">
                  ...
                  <div class="text-muted fs-7">
                    {{ t("global.Set a description to") }} {{ t(`global.${content}`) }}
                    {{ t("global.for better visibility") }}
                  </div>
                </div>
              </div>
            </languages-tabs>
            <div class="card card-flush py-4">
              <div class="card-header">
                <div class="card-title">
                  <h2 class="required">{{ t(`global.${content}`) }} {{ t("global.Name") }}</h2>
                </div>
              </div>
              <div class="card-body pt-0">
                <div class="fv-row">
                ...
                <div class="fv-row">
                  <el-form-item v-if="content === 'links'" prop="link">
                    <el-input
                      ...
                      type="text"
                      :name="`${content}-url`"
                      :placeholder="`${t(`global.${content}`)} Url`" />
                  </el-form-item>
                </div>
            ...
            <template v-if="content !== 'packages' && content !== 'links'">
...
import {defineComponent, onBeforeMount, onMounted, ref, watch, computed, nextTick} from "vue"
  ...
  setup() {
      ...
      name: [
        {
          required: true,
          trigger: "change",
          message: `A ${content} name is required and has to be unique`
        }
      ],
      status: [
        {
          required: true,
          trigger: "change",
          message: `A ${content} status is required`
        }
      ],
      department: [
        {
          required: true,
          trigger: "change",
          message: `A ${content} department is required`
        }
      ],
      thumbnail: [
        {
      ...
      link: [
        {
          required: true,
          trigger: "change",
          message: `A ${content} link is required`
        },
        {
    ...
    const handleBeforeUpload = (file) => {
      ...
      if (
        ...
        if (!allowedMimeTypes.includes(file.type)) {
          Message.error("Image must be in JPG or PNG format.")
          return false
        }
      }
      ...
      if (content === "documents") {
        ...
        if (!allowedMimeTypes.includes(file.type)) {
          Message.error("PDF must be in PDF format.")
          return false
        }
      }
    ...
    const handleOnSuccess = (response, file) => {
      ...
      // Check if the content is not "images"
      if (content !== "images") {
        // Set the default thumbnail URL for non-image content
        data.value.thumbnail.url = "/src/assets/media/upload_image.jpg"
      } else {
        // Set the thumbnail URL using the server base URL and response path
    ...
    const initializeCKEditor = (editorData) => {
      ClassicEditor.create(editorData?.editor)
        .then((editor) => {
          CKeditors.push({id: editorData?.editorId, key: editorData?.editorKey, editor})
          // eslint-disable-next-line no-console
          console.log(`Editor initialized:`, editorData?.editorKey)
        })
        .catch((error) => {
          // eslint-disable-next-line no-console
              ...
              content === "packages"
                ? {
                    ...
                    package: {
                      ...
                      // remove courses from packages
                      // our_course_count: data.value.package.ourCourses,
                ...
                : {
                    ...
                    [["selected", "dates"].join("_")]: selectedDates.value,
    ...
    const toggleCalendarVisibility = () => {
      showCalendar.value = !showCalendar.value // Toggle the value of showCalendar
    }

    const handleDateClick = (info) => {
      const dateStart = info.dateStr
      const index = selectedDates.value.indexOf(dateStart)
      if (index === -1) {
        selectedDates.value.push(dateStart)
      } else {
        selectedDates.value.splice(index, 1)
      }
      // Highlight or unhighlight the clicked date
      const dayEle = info.dayEl // Get the DOM element of the clicked date
      if (dayEle) {
        dayEle.classList.toggle("selected-date", isDateSelected(dateStart))
      }
      // Rearrange the selectedDates array
      ...
      // Add class to all dates in the selectedDates array
      selectedDates.value.forEach((date) => {
        const dateEl = document.querySelector(`.fc-day[data-date="${date}"]`)
        if (dateEl) {
    ...
    const renderFullCalendar = async () => {
      await nextTick()

      const fullCalendar = fullCalendarRef.value?.$el
      console.log(fullCalendar)

      // Check if the FullCalendar component exists
      if (fullCalendar && fullCalendar.fullCalendar) {
