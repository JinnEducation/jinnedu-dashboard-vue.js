<!-- DONE REVIEWING: NEW -->
<template>
  <toolbar :title="t('global.search-orders')" />
          ...
          <div class="card-title">
            <div class="d-flex align-items-center position-relative my-1">
              ...
              <label for="search-orders" class="sr-only">
                {{ t("global.search-orders") }}
              </label>
              <input
                id="search-orders"
                type="text"
                name="search-orders"
                :placeholder="t('global.search-orders')"
            ...
            <data-table
              ...
              <template #status="{row: order}">
                <span
                  ...
                  {{ order.status == 1 ? "Approved" : order.status == 2 ? "Failed" : "Waiting" }}
                </span>
              </template>
              ...
              <template #user="{row: order}">
                <template v-if="order.user && order.user.avatar">
                  <div class="symbol symbol-circle symbol-45px overflow-hidden me-3">
                    <div class="symbol-label">
                      <img
                        :src="`${SERVER_PATH}/${order.user.avatar}`"
                        :alt="`${order.user.name}`"
              ...
              <template v-for="type in orderTypes" :key="type">
                <div
                  ...
                  <data-table
                    ...
                    <template #student="{row: order}">
                      <template v-if="order.student">
                        <template v-if="order.student.avatar">
                          <div class="symbol symbol-circle symbol-45px overflow-hidden me-3">
                            <div class="symbol-label">
                              <img
                                :src="`${SERVER_PATH}/${order.student.avatar}`"
                    ...
                    <template #tutor="{row: order}">
                      <template v-if="order.tutor">
                        <template v-if="order.tutor.avatar">
                          <div class="symbol symbol-circle symbol-45px overflow-hidden me-3">
                            <div class="symbol-label">
                              <img
                                :src="`${SERVER_PATH}/${order.tutor.avatar}`"
                                :alt="`${order.tutor.name}`"
                                :title="`${order.tutor.name}`"
                                class="w-100" />
                            </div>
                          </div>
                        </template>
                        <template v-else>
                          <div
                            ...
                            :title="`${order.tutor.name}`">
                            <div class="symbol-label fs-3 bg-light-warning text-warning">
                              {{ order.tutor.name.charAt(0).toUpperCase() }}
                            </div>
                          </div>
                        </template>
                      </template>
...
export default defineComponent({
  ...
  setup() {
    ...
    const getDataTableBodyRows = function getDataTableBodyRows(queryString = "") {
      ...
      const url = `/orders/admin-index${queryString}${queryString ? "&" : "?"}&ref_type=${type}`
      axiosClient
        .get(url)
        .then((response) => {
    ...
    const filteredData = computed(() => {
      const selectedIndex = orderTypeIndices[selectedType.value]
      if (selectedIndex !== -1) {
        return data.value.filter((order) => order[["ref", "type"].join("_")] === selectedIndex)
      }
      // Return an empty array if the selected type is not found
      return []
    })
