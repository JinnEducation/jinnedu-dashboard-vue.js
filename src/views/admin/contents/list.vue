...
<template>
  <toolbar :title="t(`global.${content}-management`)" />
        ...
        <div class="card-header border-0 pt-6">
          <div class="card-title">
            <div class="d-flex align-items-center position-relative my-1">
              ...
              <label for="search-contents" class="sr-only">{{
                t(`global.search-${content}`)
              }}</label>
              <input
                id="search-contents"
                type="text"
                name="search-contents"
                :placeholder="t(`global.search-${content}`)"
          ...
          <div class="card-toolbar">
            <div data-kt-content-table-toolbar="base" class="d-flex justify-content-end">
              <router-link
                :to="`/dashboard/${hostArray[2]}/${content}/add`"
                ...
                {{ t("global.add-button") }} {{ t(`global.${content}`) }}
              </router-link>
            </div>
          </div>
        </div>
        ...
        <template v-else>
          <div class="card-body py-4">
            <data-table
              ...
              <template #name="{row: contentRow}">
                <template v-if="content === 'links'">
                  <a :href="contentRow.link">{{ contentRow.name }}</a>
                </template>
                <template v-else>
                  {{ contentRow.name }}
                </template>
              ...
              <template #title="{row: contentRow}">
                {{
                  contentRow.langs.find((lang) => Number(lang.language_id) === Number(languageId))
                    ?.title
                }}
              </template>

              <template v-if="content === 'events'" #start_end_date="{row: contentRow}">
                <div v-if="contentRow.selected_dates">
                  <div>
                    <span class="text-success">{{ t("global.start-date") }}:</span>
                    {{ contentRow.selected_dates[0] }}
                  </div>
                  <div>
                    <span class="text-danger">{{ t("global.end-date") }}:</span>
                    {{ contentRow.selected_dates[contentRow.selected_dates.length - 1] }}
                  </div>
                </div>
              </template>
              <!-- <template v-if="content === 'events'" #dates="{row: contentRow}">
                <div v-for="(date, index) in contentRow.selected_dates" :key="index">
                  {{ date }}
                </div>
              </template> -->

              <template #actions="{row: contentRow}">
                <router-link
                  :to="`/dashboard/${hostArray[2]}/${content}/update/${contentRow.id}`"
                  aria-label="Update"
                ...
                <button
                  ...
                  @click="deleteContent(contentRow.id)">
  ...
  setup() {
    ...
    const header = ref([
      ...
      content.value === "events" && {
        columnName: t("global.start-end-date"),
        columnLabel: "start_end_date",
        sortEnabled: true,
        columnWidth: 175
      },
      // content.value === "events" && {
      //   columnName: t("global.dates"),
      //   columnLabel: "dates",
      //   sortEnabled: true,
      //   columnWidth: 175
      // },
    ...
    const deleteContent = function deleteContent(id) {
      Swal.fire({
        icon: "error",
        ...
        if (result.isConfirmed) {
          axiosClient.delete(`/${content.value}/delete/${id}`).then(() => {
    ...
    const deleteFewContents = function deleteFewContents() {
    ...
    onBeforeRouteUpdate(async (to) => {
      content.value = to.params.content
      header.value = [
        {
          columnName: t(`global.${content.value}-name`),
          columnLabel: "name",
          sortEnabled: true,
          columnWidth: 175
        },
        {
          columnName: t(`global.${content.value}-title`),
          columnLabel: "title",
          sortEnabled: true,
          columnWidth: 175
        },
    ...
    onBeforeMount(() => {
      data.value = []
      loading.value = false
      hostArray.value = route.path.split("/")
      getDataTableBodyRows()
    })

    onMounted(() => {
      initContents.value.splice(0, data.value.length, ...data.value)
    })
