<template>
  <toolbar :title="t('global.complaints-management')" />
  <add-complaint-modal
    ref="addComplaintModal"
    :id-current="idCurrent"
    :conference-current="conferenceCurrent"
    @after-on-submit="getDataTableBodyRows" />
  <div id="kt_app_content" class="app-content flex-column-fluid">
    <div id="kt_app_content_container" class="app-container container-xxl">
      <div class="card">
        <div class="card-header border-0 pt-6">
          <div class="card-title">
            <div class="d-flex align-items-center position-relative my-1">
              <span class="svg-icon svg-icon-1 position-absolute ms-6">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  viewBox="0 0 24 24"
                  width="24"
                  height="24"
                  fill="none">
                  <rect
                    x="17"
                    y="15"
                    rx="1"
                    width="8"
                    height="2"
                    transform="rotate(45 17 15)"
                    fill="currentColor"
                    opacity="0.5" />
                  <path
                    d="M11 19C6.55556 19 3 15.4444 3 11C3 6.55556 6.55556 3 11 3C15.4444 3 19 6.55556 19 11C19 15.4444 15.4444 19 11 19ZM11 5C7.53333 5 5 7.53333 5 11C5 14.4667 7.53333 17 11 17C14.4667 17 17 14.4667 17 11C17 7.53333 14.4667 5 11 5Z"
                    fill="currentColor" />
                </svg>
              </span>
              <label for="search-complaints" class="sr-only">{{
                t("global.search-complaints")
              }}</label>
              <input
                id="search-complaints"
                type="text"
                name="search-complaints"
                :placeholder="t('global.search-complaints')"
                data-kt-complaints-table-filter="search"
                class="form-control form-control-solid w-250px ps-14"
                @keyup.enter="searchDataTableBodyRows" />
            </div>
          </div>
          <!-- make add button appear to student only -->
          <!-- <div v-if="userType === 1" class="card-toolbar">
            <div data-kt-complaint-table-toolbar="base" class="d-flex justify-content-end">
              <button type="button" class="btn btn-primary" @click="showAddModal">
                <span class="svg-icon svg-icon-2">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    viewBox="0 0 24 24"
                    width="24"
                    height="24"
                    fill="none">
                    <rect
                      x="11"
                      y="20"
                      rx="1"
                      width="16"
                      height="2"
                      transform="rotate(-90 11 20)"
                      fill="currentColor"
                      opacity="0.5" />
                    <rect x="4" y="11" rx="1" width="16" height="2" fill="currentColor" />
                  </svg>
                </span>
                {{ t("global.add-button") }} {{ t("global.complaint") }}
              </button>
            </div>
          </div> -->
        </div>
        <template v-if="loading">
          <div class="card-body p-0">
            <div class="card-px text-center py-20 my-10">
              <div role="status" class="spinner-border text-primary">
                <span class="visually-hidden">{{ t("global.loading") }}...</span>
              </div>
            </div>
          </div>
        </template>
        <template v-else-if="data.length === 0">
          <div class="card-body p-0">
            <div class="card-px text-center py-20 my-10">
              <h2 class="fs-2x fw-bold mb-10">{{ t("global.welcome") }}</h2>
              <p class="text-gray-400 fs-5 fw-semibold mb-13">
                <span>
                  {{ t("global.no-complaints") }}
                </span>
              </p>
              <button type="button" class="btn btn-primary er fs-6 px-8 py-4" @click="showAddModal">
                {{ t("global.add-button") }} {{ t("global.complaint") }}
              </button>
            </div>
            <div class="text-center px-5">
              <img
                src="@/assets/media/illustrations/welcome.png"
                :alt="`Add Complaints Illustration`"
                class="mw-100 mh-300px" />
            </div>
          </div>
        </template>
        <template v-else>
          <div class="card-body py-4">
            <data-table
              :data="data"
              :header="header"
              :checkbox-enabled="false"
              checkbox-label="id"
              :items-total="itemsTotal"
              :page-current="currentPage"
              :items-per-page="itemsPerPage"
              :items-per-page-dropdown-enabled="true"
              :query-string="currentSearchQuery"
              @on-sort="onSort"
              @on-items-select="onItemsSelect">
              <template #id="{row: complaint}">
                {{ data.indexOf(complaint) + 1 }}
              </template>

              <template #subject="{row: complaint}">
                <div class="d-flex mb-10 p-1">
                  <div class="d-flex flex-column">
                    <div class="d-flex align-items-center mb-2">
                      <template v-if="userType === 0 || userType === 1">
                        <router-link
                          :to="`/dashboard/conferences/${complaint.conference_id}/complaints/${complaint.id}`"
                          class="text-dark text-hover-primary fs-4 me-3 fw-semibold">
                          {{ complaint.subject }}
                        </router-link>
                      </template>
                      <template v-else>
                        <span class="text-dark text-hover-primary fs-4 me-3 fw-semibold">
                          {{ complaint.subject }}
                        </span>
                      </template>
                      <span v-if="complaint.status === 1" class="badge badge-success me-auto">{{
                        t("global.opened")
                      }}</span>
                      <span v-else-if="complaint.status === 2" class="badge badge-light me-auto">{{
                        t("global.pending")
                      }}</span>
                      <span v-else-if="complaint.status === 3" class="badge badge-info me-auto">{{
                        t("global.resolved")
                      }}</span>
                      <span
                        v-else-if="complaint.status === 4"
                        class="badge badge-light-danger me-auto"
                        >{{ t("global.closed") }}</span
                      >
                      <span
                        v-else-if="complaint.status === 5"
                        class="badge badge-light-warning me-auto"
                        >{{ t("global.accepted") }}</span
                      >
                      <span
                        v-else-if="complaint.status === 6"
                        class="badge badge-light-danger me-auto"
                        >{{ t("global.rejected") }}</span
                      >
                      <span v-else class="badge badge-secondary me-auto">{{
                        t("global.unknown")
                      }}</span>
                    </div>
                    <span class="text-muted fw-semibold fs-6">{{ complaint.note }}</span>
                  </div>
                </div>
              </template>
              <template #conference="{row: complaint}">
                {{ complaint.conference.title }}
              </template>
              <template #student="{row: complaint}">
                {{ complaint.student.name }}
              </template>
              <template #tutor="{row: complaint}">
                {{ complaint.tutor.name }}
              </template>

              <template #actions="{row: complaint}">
                <a
                  v-if="complaint ? (complaint.path !== null ? true : false) : false"
                  :href="`${SERVER_PATH}/${complaint.path}`"
                  aria-label="Delete"
                  class="btn btn-icon btn-light-success me-2">
                  <span class="svg-icon svg-icon-success">
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      viewBox="0 0 24 24"
                      style="width: 1.5rem; height: 1.5rem">
                      <g fill="none" fill-rule="evenodd">
                        <rect x="0" y="0" width="24" height="24" />
                        <path
                          d="M2,13 C2,12.5 2.5,12 3,12 C3.5,12 4,12.5 4,13 C4,13.3333333 4,15 4,18 C4,19.1045695 4.8954305,20 6,20 L18,20 C19.1045695,20 20,19.1045695 20,18 L20,13 C20,12.4477153 20.4477153,12 21,12 C21.5522847,12 22,12.4477153 22,13 L22,18 C22,20.209139 20.209139,22 18,22 L6,22 C3.790861,22 2,20.209139 2,18 C2,15 2,13.3333333 2,13 Z"
                          fill="currentColor"
                          fill-rule="nonzero"
                          opacity="0.5" />
                        <rect
                          x="11"
                          y="1"
                          rx="1"
                          width="2"
                          height="14"
                          transform="translate(12, 8) rotate(-180) translate(-12, -8)"
                          fill="currentColor"
                          opacity="0.5" />
                        <path
                          d="M7.70710678,15.7071068 C7.31658249,16.0976311 6.68341751,16.0976311 6.29289322,15.7071068 C5.90236893,15.3165825 5.90236893,14.6834175 6.29289322,14.2928932 L11.2928932,9.29289322 C11.6689749,8.91681153 12.2736364,8.90091039 12.6689647,9.25670585 L17.6689647,13.7567059 C18.0794748,14.1261649 18.1127532,14.7584547 17.7432941,15.1689647 C17.3738351,15.5794748 16.7415453,15.6127532 16.3310353,15.2432941 L12.0362375,11.3779761 L7.70710678,15.7071068 Z"
                          transform="translate(12, 12) rotate(-180) translate(-12, -12)"
                          fill="currentColor"
                          fill-rule="nonzero" />
                      </g>
                    </svg>
                  </span>
                </a>
                <!-- make delete button appear to student only -->
                <button
                  v-if="userType === 1"
                  type="button"
                  aria-label="Delete"
                  class="btn btn-icon btn-light-danger"
                  @click="deleteComplaint(complaint.id)">
                  <span class="svg-icon svg-icon-danger">
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      viewBox="0 0 24 24"
                      style="width: 1.5rem; height: 1.5rem">
                      <path
                        d="M6,8 L18,8 L17.106535,19.6150447 C17.04642,20.3965405 16.3947578,21 15.6109533,21 L8.38904671,21 C7.60524225,21 6.95358004,20.3965405 6.89346498,19.6150447 L6,8 Z M8,10 L8.45438229,14.0894406 L15.5517885,14.0339036 L16,10 L8,10 Z"
                        fill="currentColor"
                        fill-rule="nonzero" />
                      <path
                        d="M14,4.5 L14,3.5 C14,3.22385763 13.7761424,3 13.5,3 L10.5,3 C10.2238576,3 10,3.22385763 10,3.5 L10,4.5 L5.5,4.5 C5.22385763,4.5 5,4.72385763 5,5 L5,5.5 C5,5.77614237 5.22385763,6 5.5,6 L18.5,6 C18.7761424,6 19,5.77614237 19,5.5 L19,5 C19,4.72385763 18.7761424,4.5 18.5,4.5 L14,4.5 Z"
                        fill="currentColor"
                        opacity="0.5" />
                    </svg>
                  </span>
                </button>
              </template>
            </data-table>
          </div>
        </template>
      </div>
    </div>
  </div>
</template>

<script>
import Toolbar from "@/components/admin/dashboard/toolbar.vue"
import DataTable from "@/components/admin/data-table/index.vue"
import AddComplaintModal from "@/components/admin/modals/forms/add-complaint-modal.vue"
import axiosClient from "@/plugins/axios"
import arraySort from "array-sort"
import {defineComponent, onBeforeMount, onMounted, provide, ref} from "vue"
import {useI18n} from "vue-i18n"
import {useRoute} from "vue-router"
import {useStore} from "vuex"
import {showModal} from "../../../core/helpers/dom"

export default defineComponent({
  name: "add-complaint-list",
  components: {Toolbar, AddComplaintModal, DataTable},
  setup() {
    const SERVER_PATH = ref(import.meta.env.VITE_APP_SERVER_BASE_URL)
    const route = useRoute()
    const store = useStore()
    const {userInfo} = store.state
    const userInfoObject = JSON.parse(userInfo)
    const userType = userInfoObject.user.type
    const {t} = useI18n()
    const conference = ref(route.params.conference)
    const loading = ref(false)
    const conferenceId = route.params.conference_id
    const header = ref([
      {
        columnName: "#",
        columnLabel: "id",
        sortEnabled: true,
        columnWidth: 75
      },

      {
        columnName: t("global.subject"),
        columnLabel: "subject",
        sortEnabled: true,
        columnWidth: 175
      },
      {
        columnName: t("global.conference"),
        columnLabel: "conference",
        sortEnabled: true,
        columnWidth: 75
      },
      {
        columnName: t("global.student"),
        columnLabel: "student",
        sortEnabled: true,
        columnWidth: 175
      },
      {
        columnName: t("global.tutor"),
        columnLabel: "tutor",
        sortEnabled: true,
        columnWidth: 175
      },

      {
        columnName: t("global.actions"),
        columnLabel: "actions",
        sortEnabled: false,
        columnWidth: 175
      }
    ])

    const currentSearchQuery = ref("")

    const data = ref([])
    const itemsTotal = ref(0)
    const currentPage = ref(0)
    const itemsPerPage = ref(0)
    const initConferences = ref([])
    const idsSelected = ref([])
    const addComplaintModal = ref(null)
    const idCurrent = ref(null)
    const conferenceCurrent = ref({id: null})

    const getDataTableBodyRows = function getDataTableBodyRows(queryString = "") {
      loading.value = true
      if (userType === 0) {
        // admin
        axiosClient
          .get(`/complaints?${queryString}&conference_id=${conferenceId}`)
          .then((response) => {
            data.value = response.data.result.data
            itemsTotal.value = response.data.result.total
            currentPage.value = response.data.result.current_page
            itemsPerPage.value = response.data.result.per_pages
          })
          .finally(() => {
            loading.value = false
          })
      } else if (userType === 1) {
        // student
        axiosClient
          .get(`/complaints?${queryString}&student_id=${userInfoObject.user.id}`)
          .then((response) => {
            data.value = response.data.result.data
            itemsTotal.value = response.data.result.total
            currentPage.value = response.data.result.current_page
            itemsPerPage.value = response.data.result.per_pages
          })
          .finally(() => {
            loading.value = false
          })
      } else if (userType === 2) {
        // tutor
        axiosClient
          .get(`/complaints?${queryString}&tutor_id=${userInfoObject.user.id}`)
          .then((response) => {
            data.value = response.data.result.data
            itemsTotal.value = response.data.result.total
            currentPage.value = response.data.result.current_page
            itemsPerPage.value = response.data.result.per_pages
          })
          .finally(() => {
            loading.value = false
          })
      }
    }

    const searchDataTableBodyRows = function searchDataTableBodyRows(e) {
      currentSearchQuery.value = e.target.value
      getDataTableBodyRows(`q=${currentSearchQuery.value}`)
    }

    const deleteComplaint = function deleteComplaint(id) {
      Swal.fire({
        icon: "error",
        text: t("global.ensure-delete"),
        showCancelButton: true,
        confirmButtonText: t("global.yes-delete"),
        cancelButtonText: t("global.go-back"),
        buttonsStyling: false,
        customClass: {confirmButton: "btn btn-danger", cancelButton: "btn btn-active-light"}
      }).then((result) => {
        if (result.isConfirmed) {
          axiosClient.delete(`/conferences/add-complaint/delete/${id}`).then(() => {
            getDataTableBodyRows()
          })
        }
      })
    }

    const deleteFewComplaints = function deleteFewComplaints() {
      idsSelected.value.forEach((item) => {
        deleteComplaint(item)
      })

      idsSelected.value.length = 0
    }

    const onSort = function onSort(sort) {
      const reverse = sort.order === "ASC".toLowerCase()
      if (sort.label) arraySort(data.value, sort.label, {reverse})
    }

    const onItemsSelect = function onItemsSelect(itemsSelected) {
      if (itemsSelected.length === 0) idsSelected.value = []
      else idsSelected.value = [...idsSelected.value, ...itemsSelected]
    }

    const showAddModal = function showAddModal() {
      idCurrent.value = conference.value
      console.log(idCurrent.value)
      conferenceCurrent.value = {id: conference.value}
      showModal(addComplaintModal.value.addComplaintModal.modal)
    }

    const showUpdateModal = function showUpdateModal(id) {
      idCurrent.value = conference.value
      axiosClient
        .get(`/conferences/add-complaint/${conference.value}/show/${id}`)
        .then((response) => {
          conferenceCurrent.value = response.data.result
          showModal(addComplaintModal.value.addComplaintModal.modal)
        })
    }

    provide("getDataTableBodyRows", getDataTableBodyRows)
    onBeforeMount(() => {
      data.value = []
      loading.value = false
      getDataTableBodyRows()
    })

    onMounted(() => {
      initConferences.value.splice(0, data.value.length, ...data.value)
    })

    return {
      SERVER_PATH,
      t,
      userType,
      conferenceId,
      conference,
      loading,
      header,
      data,
      itemsTotal,
      currentPage,
      itemsPerPage,
      idsSelected,
      addComplaintModal,
      idCurrent,
      conferenceCurrent,
      currentSearchQuery,
      searchDataTableBodyRows,
      onSort,
      onItemsSelect,
      deleteComplaint,
      deleteFewComplaints,
      showAddModal,
      showUpdateModal,
      getDataTableBodyRows
    }
  }
})
</script>
