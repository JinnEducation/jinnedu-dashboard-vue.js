...
<template>
        ...
        <div class="card-header border-0 pt-6">
          <div class="card-title">
            <div class="d-flex align-items-center position-relative my-1">
              ...
              <input id="search-users" type="text" name="search-users" :placeholder="t('global.search-users')"
              ...
              <div :id="`${type.toLowerCase().replace(/\s/g, '-')}-tab-content`" class="tab-pane"
                ...
                <data-table v-if="selectedType === type" :data="data" :header="header" :checkbox-enabled="true"
                  ...
                  <template #actions="{ row: user }">
                    <button type="button" class="btn btn-icon btn-light-info p-3 me-2 userchattooltip user-chat-btn"
                      @click="gotoUserChat(user.id)">
                      <svg class="user-chat-svg" width="24" height="24" viewBox="0 0 24 24" fill="currentColor"
                    ...
                    <button v-if="user.type !== 0 && userPermissionsInUserModule.includes('index')" type="button"
                      class="btn btn-light-primary p-3 me-2" @click="
                        user.type === 2
                    ...
                    <button v-if="userPermissionsInUserModule.includes('index')" type="button" aria-label="Chat"
                      class="btn btn-icon btn-light-warning me-2 chattooltip chat-btn" @click="chat(user.id)">
                      <svg class="w-6 h-6 dark:text-white chat-svg" aria-hidden="true"
                    ...
                    <button v-if="userPermissionsInUserModule.includes('edit')" type="button" aria-label="Update"
                      class="btn btn-icon btn-light-success edittooltip me-2" @click="showUpdateModal(user.id)">
                    ...
                    <button v-if="userPermissionsInUserModule.includes('destroy')" type="button" aria-label="Delete"
                      class="btn btn-icon btn-light-danger deletetooltip" @click="deleteUser(user.id)">
  ...
  <add-user-modal ref="addUserModal" :id-current="idCurrent" :user-current="userCurrent" :selected-type="selectedType"
      ...
      if (Array.isArray(userData?.roles)) {
        userData.roles.forEach(role => {
          if (Array.isArray(role?.permissions)) {
            ...
            if (matchingPermission && Array.isArray(matchingPermission.childrens)) {
              matchingPermission.childrens.forEach(child => {
                if (child.checked === true) {
                  permissionWithCheckedChildren.add(child.name)
                }
    ...
    const showUpdateModal = function showUpdateModal(id) {
      ...
      axiosClient.get(`/users/show/${id}`).then((response) => {
        ...
        if (userCurrent.value.avatar !== "" && userCurrent.value.avatar !== null)
          document.getElementById("add-user-modal-avatar").style.backgroundImage = `url('${import.meta.env.VITE_APP_SERVER_BASE_URL
