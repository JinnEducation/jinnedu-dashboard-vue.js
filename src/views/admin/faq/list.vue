        ...
        <div class="card-header border-0 pt-6">
          <div class="card-title">
            <div class="d-flex align-items-center position-relative my-1">
              ...
              <label for="search-faq" class="sr-only">{{ t("global.search") }}</label>
              <input
                id="search-faq"
                type="text"
                name="search-faq"
          ...
          <div class="card-toolbar">
            <div data-kt-content-table-toolbar="base" class="d-flex justify-content-end">
              <template v-if="abilities.create">
                <router-link :to="`/dashboard/faq/create`" class="btn btn-primary">
                  ...
                  {{ t("global.add-button") }} {{ t("global.faq") }}
                </router-link>
              </template>
            </div>
          </div>
            ...
            <data-table
              ...
              <template #question="{row: item}">
                <div class="fw-bold text-gray-800">
                  {{ getTranslatedValue(item.question) || "-" }}
                </div>
              </template>

              <template #status="{row: item}">
                <span v-if="item.status === 'published'" class="badge badge-light-success">
                  {{ t("global.published") }}
                </span>
                <span v-else-if="item.status === 'draft'" class="badge badge-light-warning">
                  {{ t("global.draft") }}
                </span>
              ...
              <template #sort_order="{row: item}">
                <span class="badge badge-light-info">{{ item.sort_order ?? 0 }}</span>
              </template>

              <template v-if="abilities.edit || abilities.destroy" #actions="{row: item}">
                <router-link
                  v-if="abilities.edit"
                  :to="`/dashboard/faq/update/${item.id}`"
                  aria-label="Update"
...
export default defineComponent({
  ...
  setup() {
    ...
    const getDataTableBodyRows = function getDataTableBodyRows(queryString = "") {
      ...
      axiosClient
        ...
        .then((response) => {
          ...
          itemsPerPage.value = response.data.meta?.per_pages || 10
        })
        .finally(() => {
          loading.value = false
        })
    ...
    const deleteFaq = function deleteFaq(id) {
      Swal.fire({
        icon: "error",
        ...
        if (result.isConfirmed || result.value) {
          axiosClient.delete(`/faq/${id}`).then(() => {
            Swal.fire({
              icon: "success",
    ...
    onMounted(() => {
      initFaq.value.splice(0, data.value.length, ...data.value)
    })
