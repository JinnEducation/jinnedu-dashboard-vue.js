...
<template>
  <toolbar :title="t('global.cateqblog-management')" />
        ...
        <div class="card-header border-0 pt-6">
          <div class="card-title">
            <div class="d-flex align-items-center position-relative my-1">
              ...
              <label for="search-cateqblog" class="sr-only">{{ t("global.search-cateqblog") }}</label>
              <input id="search-cateqblog" type="text" name="search-cateqblog" :placeholder="t('global.search-cateqblog')"
          ...
          <div class="card-toolbar">
            <div data-kt-content-table-toolbar="base" class="d-flex justify-content-end">
              <template v-if="abilities.create">
                <router-link :to="`/dashboard/cateqblog/create`" class="btn btn-primary">
                  ...
                  {{ t("global.add-button") }} {{ t("global.cateqblog") }}
                </router-link>
              </template>
            </div>
          </div>
          ...
          <div class="card-body p-0">
            <div class="card-px text-center py-20 my-10">
              ...
              <p class="text-gray-400 fs-5 fw-semibold mb-13">
                <span>
                  {{ t("global.no-cateqblog") }}
                </span>
              </p>
              <template v-if="abilities.create">
                <router-link :to="`/dashboard/cateqblog/create`" class="btn btn-primary er fs-6 px-8 py-4">
                  {{ t("global.add-button") }} {{ t("global.cateqblog") }}
                </router-link>
              </template>
            </div>
            <div class="text-center px-5">
              <img src="@/assets/media/illustrations/welcome.png" :alt="`Add Our cateqblog Illustration`"
        ...
        <template v-else>
          <div class="card-body py-4">
            <data-table :data="data" :header="header" :checkbox-enabled="true" checkbox-label="id"
              ...
              <!--  والعنوان -->
              <template #name="{ row: post }">
                <div class="d-flex align-items-center">
                  <div>
                    <div class="fw-bold">
                      {{ typeof post.name === 'object' ? post.name[languageId] : post.name }}
                    </div>
              ...
              <!-- sort_order -->
              <template #sort_order="{ row: post }">
                {{ post.sort_order }}
              </template>
              <!-- is_active -->
              <template #is_active="{ row: post }">
                <span v-if="post.is_active === 1" class="text-success">Active</span>
                <span v-else class="text-muted">Inactive</span>
              </template>

              <!-- الأزرار -->
              <template v-if="abilities.edit || abilities.destroy" #actions="{ row: post }">
                <router-link v-if="abilities.edit" :to="`/dashboard/cateqblog/update/${post.id}`" aria-label="Update"
                ...
                <button v-if="abilities.destroy" type="button" aria-label="Delete"
                  class="btn btn-icon btn-light-danger deletetooltip" @click="deletecateqblog(post.id)">
...
export default defineComponent({
  ...
  setup() {
    ...
    const header = ref([
      ...
      {
        columnName: t("global.sort_order"),
        columnLabel: "sort_order",
        sortEnabled: true,
        columnWidth: 160
      },
      {
        columnName: t("global.is_active"),
    ...
    const deletecateqblog = (id) => {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          axiosClient.delete(`/cateqblog/${id}`).then(() => {
            Swal.fire({
              icon: "success",
    ...
    const deleteFewOurcateqblog = function deleteFewOurcateqblog() {
      idsSelected.value.forEach((item) => {
        deletecateqblog(item)
      })

      idsSelected.value.length = 0
    }

    const registercateqblog = function registercateqblog(id) {
      loading.value = true
      axiosClient
        .post(`/cateqblog/register-as-tutor/${id}`)
        .then(() => {
          getDataTableBodyRows()
          Swal.fire({
            icon: "success",
    ...
    onMounted(() => {
      initOurcateqblog.value.splice(0, data.value.length, ...data.value)
    })
