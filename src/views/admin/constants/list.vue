        ...
        <div class="card-header border-0 pt-6">
          <div class="card-title">
            <div class="d-flex align-items-center position-relative my-1">
              ...
              <label for="search-constants" class="sr-only">{{
                t("global.search-constants")
              }}</label>
              <input
                id="search-constants"
                type="text"
                name="search-constants"
                :placeholder="t('global.search') + ' ' + t(`global.${constant}`)"
          ...
          <div class="card-toolbar">
            <div data-kt-constant-table-toolbar="base" class="d-flex justify-content-end">
              <button type="button" class="btn btn-primary" @click="showAddModal">
                ...
                {{ t("global.add-button") }} {{ t(`global.${constant}`) }}
              </button>
            </div>
          </div>
        </div>
            ...
            <data-table
              ...
              <template #actions="{row: constantRow}">
                <button
                  ...
                  @click="showUpdateModal(constantRow.id)">
                ...
                <button
                  ...
                  @click="deleteConstant(constantRow.id)">
...
export default defineComponent({
  ...
  setup() {
    ...
    const header = ref([
      {
        columnName: t(`global.${constant.value}-name`),
        columnLabel: "name",
        sortEnabled: true,
        columnWidth: 175
      },
      {
        columnName: t(`global.${constant.value}-title`),
        columnLabel: "title",
        sortEnabled: true,
        columnWidth: 175
      },
    ...
    const getDataTableBodyRows = function getDataTableBodyRows(queryString = "") {
      loading.value = true
      axiosClient
        .get(`/${constant.value}${queryString}`)
        .then((response) => {
