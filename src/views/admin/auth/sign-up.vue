<template>
  <div
    ...
    <div>
      <div class="mr-2 previous-button">
        <button
          ...
          <form
              ...
              <template v-if="!isProfile">
                <div class="w-100">
                  <div class="pb-10 pb-lg-15">
                    <h2
                      class="fw-bold d-flex align-items-center justify-content-center text-dark mb-20">
                      {{ t("global.account-type-title") }}
                    </h2>
                    <div class="fw-semibold fs-6 text-black">
                      {{ t("global.choose-your-account-type") }}
                    </div>
                  </div>
                  <div class="fv-row">
                    <div class="row">
            ...
            <div data-kt-stepper-element="content">
              <div class="w-100">
                ...
                <div class="row fv-row mb-7">
                  <div class="col-xl-6">
                    <label for="first-name" class="form-label required-star">{{
                      t("global.first-name")
                    }}</label>
                    <form-input
                      id="first-name"
                  ...
                  <div class="col-xl-6">
                    <label for="last-name" class="form-label required-star">{{
                      t("global.last-name")
                    }}</label>
                    <form-input
                      id="last-name"
                ...
                <div class="fv-row mb-10">
                  <label for="email" class="form-label required-star">{{
                    t("global.email")
                  }}</label>
                  <form-input
                    id="email"
                ...
                <div class="fv-row mb-10">
                  <div class="d-flex">
                    <div class="d-flex flex-column country-code-select">
                      <label for="country-code" class="form-label required-star">{{
                        t("global.country")
                      }}</label>
                ...
                <div
                  ...
                  <div class="mb-1">
                    <div class="position-relative mb-3">
                      <label for="password" class="form-label required-star">{{
                        t("global.password")
                      }}</label>
                      <form-input
                        v-if="!isProfile"
                      ...
                      <span
                        ...
                        class="btn btn-sm btn-icon position-absolute translate-middle top-65 end-0 me-n2">
                ...
                <div v-if="!isProfile" class="fv-row mb-10">
                  <label for="confirm-password" class="form-label required-star">{{
                    t("global.confirm-password")
                  }}</label>
                  <form-input
                    v-if="!isProfile"
                    id="confirm-password"
                ...
                <div v-if="data.accountType === 'student' && !isProfile" class="fv-row mb-10">
            ...
            <div v-show="data.accountType === 'tutor'" data-kt-stepper-element="content">
              <div class="w-100">
                ...
                <!-- Add Terms and Conditions Checkbox -->
                <div v-if="data.accountType === 'tutor' && !isProfile" class="fv-row mb-10 mt-10">
                  <div class="form-check">
                    <input
                      id="agree-terms"
                      v-model="data.agreeTerms"
              ...
              <div>
                <button
                  ...
                  <img
                    ...
                    class="h-25px rounded me-5" />
                  {{ t("global.google") }}
                </button>
              </div>
              <div>
                <button
                  v-show="(navigation.submit && data.agreeTerms) || isProfile"
 ...
 -->
                  <span class="indicator-label">
                    {{ isProfile ? t("global.update-profile") : t("global.create-account") }}
...
export default defineComponent({
  ...
  setup() {
    ...
    const handleFormSubmission = function handleFormSubmission() {
      validation[stepperInstance.getCurrentStepIndex() - 1].validate().then((status) => {
        if (status.toString().toLowerCase() === "valid") {
          ...
          formData.append("phone", data.value.phoneNumber)
          formData.append("countryCode", data.value.countryCode)
          formData.append("phoneCountryId", data.value.phoneCountryId)
          // }

          if (!isProfile) {
            formData.append("agreeTerms", data.value.agreeTerms)
          ...
          store
            ...
            .then((response) => {
              formButtonSubmit.value.setAttribute("data-kt-indicator", "off")

              if (isProfile && data.value.accountType === "student" && response.success) {
                Swal.fire({
                  icon: "success",
                                    ...
                                    Promise.allSettled([certificatePromise])
                                      .then(() => {
                                        ...
                                        Promise.allSettled([videoPromise])
                                          ...
                                          .finally(() => {
                                            formButtonSubmit.value.setAttribute(
                                              "data-kt-indicator",
                                              "off"
                                            )
                                            // formButtonSubmit.value.setAttribute(
    ...
    const getCountryData = async () => {
      try {
        ...
        if (countries.value.length > 0) {
          data.value.countryCode = countries.value[0].dialCode
          updateCountry()
          // data.value.phoneCountryId = countries.value[0].id
        }
      } catch (error) {
    ...
    const updateCountry = () => {
      ...
      if (selectedCountry) {
        data.value.phoneCountryId = selectedCountry.id
      }
      validatePhone()
    }

    // Watch for changes in phone number
    watch(() => data.value.phoneNumber, validatePhone)
