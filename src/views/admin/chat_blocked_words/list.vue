...
<template>
  <toolbar :title="t('global.chat-blocked-words-management')" />
        ...
        <div class="card-header border-0 pt-6">
          <div class="card-title">
            <div class="d-flex align-items-center position-relative my-1">
              ...
              <label for="search-blocked-word" class="sr-only">{{
                t("global.search-blocked-word")
              }}</label>
              <input
                id="search-blocked-word"
                type="text"
                name="search-blocked-word"
                :placeholder="t('global.search-blocked-word')"
          ...
          <div class="card-toolbar">
            <div data-kt-content-table-toolbar="base" class="d-flex justify-content-end">
              <template v-if="abilities.create">
                <router-link :to="`/dashboard/chat_blocked_words/create`" class="btn btn-primary">
                  ...
                  {{ t("global.add-button") }} {{ t("global.chat-blocked-word") }}
                </router-link>
              </template>
            </div>
          </div>
          ...
          <div class="card-body p-0">
            <div class="card-px text-center py-20 my-10">
              ...
              <p class="text-gray-400 fs-5 fw-semibold mb-13">
                <span>
                  {{ t("global.no-chat-blocked-words") }}
                </span>
              </p>
              <template v-if="abilities.create">
                <router-link
                  :to="`/dashboard/chat_blocked_words/create`"
          ...
          <div class="card-body py-4">
            <data-table
              ...
              <template v-if="abilities.edit || abilities.destroy" #actions="{row: item}">
                <router-link
                  v-if="abilities.edit"
                  :to="`/dashboard/chat_blocked_words/update/${item.id}`"
                  aria-label="Update"
                ...
                <button
                  ...
                  @click="deleteBlockedWord(item.id)">
...
export default defineComponent({
  ...
  setup() {
    ...
    const getDataTableBodyRows = function getDataTableBodyRows(queryString = "") {
      loading.value = true
      axiosClient
        .get(`/chat-blocked-words${queryString}`)
        .then((response) => {
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          axiosClient.delete(`/chat-blocked-words/${id}`).then(() => {
            Swal.fire({
              icon: "success",
              text: t("global.chat-blocked-word-deleted-successfully"),
    ...
    const deleteFewBlockedWords = function deleteFewBlockedWords() {
      idsSelected.value.forEach((item) => {
        deleteBlockedWord(item)
      })
