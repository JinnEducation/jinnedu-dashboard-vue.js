<template>
  <toolbar :title="t('global.student-information')" />
          ...
          <div class="row">
            <div class="col-md-3">
              <div class="card-body pt-9 pb-0">
                <div v-if="profile" class="d-flex flex-wrap flex-sm-nowrap mb-6">
                  <div
            ...
            <div class="col-md-4">
              <div v-if="!loading">
                ...
                <template v-if="profile.abouts[0]">
                  ...
                  <p>
                    <strong>{{ t("global.last_online_date") }}:</strong>
                    {{ profile.last_online_date }}
                  </p>
                  <p>
                    <strong>{{ t("global.status") }}:</strong>
                    {{ profile.abouts[0]?.online == 1 ? "online" : "offline" }}
                  </p>
                </template>
                <template v-else>
                  <p>
...
export default defineComponent({
  ...
  setup() {
    ...
    const getDataTableBodyRows = async (queryString = "") => {
      ...
      const urls = {
        "Completed Group Class": "/student-statistics/group-class-orders",
        "Not Completed Group Class": "/student-statistics/group-class-orders",
        "Completed Private Lesson": "/student-statistics/private-lesson-orders",
        "Not Completed Private Lesson": "/student-statistics/private-lesson-orders"
      ...
      let url = `${urls[selectedType.value]}?student_id=${userId}`
    ...
    onBeforeMount(async () => {
      try {
        ...
        const [profileResponse, userInfoResponse] = await Promise.all([
          ...
          axiosClient.get("/student-statistics/student-info", {params: {user_id: userId}})
        ])
