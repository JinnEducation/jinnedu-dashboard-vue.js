        ...
        <div class="card-header border-0 pt-6">
          <div class="card-title">
            <div class="d-flex align-items-center position-relative my-1">
              ...
              <label for="search-reviews" class="sr-only">{{ t("global.search-reviews") }}</label>
              <input
                id="search-reviews"
                type="text"
                name="search-reviews"
                :placeholder="t('global.search-reviews')"
                data-kt-reviews-table-filter="search"
                class="form-control form-control-solid w-250px ps-14"
                @keyup.enter="searchDataTableBodyRows" />
            </div>
          </div>
          <div class="card-toolbar">
            <div data-kt-review-table-toolbar="base" class="d-flex justify-content-end">
              <button type="button" class="btn btn-primary" @click="showAddModal">
                ...
                {{ t("global.add-button") }} {{ t("global.review") }}
              </button>
            </div>
          </div>
        </div>
        ...
        <template v-else>
          <div class="card-body py-4">
            <data-table
              ...
              <template #comment="{row: review}">
                {{ review.comment }}
              </template>
              <template #actions="{row: review}">
                <button
                  type="button"
                  aria-label="Update"
                  class="btn btn-icon btn-light-success me-2"
                  @click="showUpdateModal(review.id)">
...
export default defineComponent({
  ...
  setup() {
    ...
    const getDataTableBodyRows = function getDataTableBodyRows(queryString = "") {
      loading.value = true
      axiosClient
        .get(`/front/reviews/list/conference/${conference.value}${queryString}`)
        .then((response) => {
    ...
    const deleteReview = function deleteReview(id) {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          axiosClient.delete(`/front/reviews/delete/${id}`).then(() => {
    ...
    const showUpdateModal = function showUpdateModal(id) {
      idCurrent.value = conference.value
      axiosClient.get(`/front/reviews/show/${id}`).then((response) => {
