<!-- DONE REVIEWING: NEW -->
<template>
  <toolbar :title="t('global.roles-management')" />
        ...
        <div class="card-header border-0 pt-6">
          <div class="card-title">
            <div class="d-flex align-items-center position-relative my-1">
              ...
              <label for="search-roles" class="sr-only">{{ t("global.search-roles") }}</label>
              <input
                id="search-roles"
                type="text"
                name="search-roles"
                :placeholder="t('global.search-roles')"
          ...
          <div class="card-toolbar">
            <div data-kt-role-table-toolbar="base" class="d-flex justify-content-end">
              <router-link :to="`/dashboard/roles/add`" class="btn btn-primary">
                ...
                {{ t("global.add-button") }} {{ t("global.role") }}
              </router-link>
            </div>
          </div>
        </div>
        ...
        <template v-else-if="data.length === 0">
          <div class="card-body p-0">
            <div class="card-px text-center py-20 my-10">
              ...
              <router-link :to="`/dashboard/roles/add`" class="btn btn-primary er fs-6 px-8 py-4">
                {{ t("global.add-button") }} {{ t("global.role") }}
              </router-link>
            </div>
        ...
        <template v-else>
          <div class="card-body py-4">
            <data-table
              ...
              <template #id="{row: role}">
                {{ data.indexOf(role) + 1 }}
              </template>
              <template #name="{row: role}">
              ...
              <template #actions="{row: role}">
                <router-link
                  :to="`/dashboard/roles/update/${role.id}`"
                  aria-label="Update"
...
export default defineComponent({
  ...
  setup() {
    ...
    const deleteRole = function deleteRole(id) {
      Swal.fire({
        icon: "error",
        ...
        if (result.isConfirmed) {
          axiosClient.delete(`/roles/delete/${id}`).then(() => {
    ...
    const deleteFewRoles = function deleteFewRoles() {
    ...
    onMounted(() => {
      initRoles.value.splice(0, data.value.length, ...data.value)
    })
