<template>
  <div>
    <toolbar :title="t('global.tutor-finances-management')" />
        ...
        <div class="card">
          ...
          <div class="card-header border-0 pt-6">
            <div class="card-title">
              <div class="d-flex align-items-center position-relative my-1">
                ...
                <label for="search-tutor-finances" class="sr-only">
                  {{ t("global.search-tutor-finances") }}
                </label>
                <input id="search-tutor-finances" type="text" name="search-tutor-finances"
                  :placeholder="t('global.search-tutor-finances')" data-kt-word-table-filter="search"
          ...
          <template v-else-if="data.length === 0">
            <div class="card-body p-0">
              ...
              <div class="text-center px-5">
                <img src="@/assets/media/illustrations/welcome.png" alt="Add User Illustration"
          ...
          <template v-else>
            <div class="card-body py-4">
              <data-table :data="data" :header="header" :checkbox-enabled="false" checkbox-label="id"
                ...
                <template #tutor="{ row: tutorFinance }">
                  <div class="d-flex align-items-center justify-content-start">
                    <template v-if="tutorFinance?.tutor?.avatar !== '' && tutorFinance?.tutor?.avatar !== null">
                      <div class="symbol symbol-circle symbol-50px overflow-hidden me-3">
                        <div class="symbol-label">
                          <img :src="tutorFinance?.tutor?.avatar" :alt="`${tutorFinance?.tutor?.full_name}`"
                            class="w-100" />
                        </div>
                      </div>
                    </template>
                    <template v-else>
                      <div class="symbol symbol-circle symbol-50px overflow-hidden me-3"
                        ...
                        <div class="symbol-label fs-3 bg-light-warning text-warning">
                          {{ tutorFinance?.tutor?.full_name?.charAt(0)?.toUpperCase() || '' }}
                        </div>
                      </div>
                    </template>
                    {{ tutorFinance?.tutor?.full_name }}
                  </div>
                </template>
                <template #amount="{ row: tutorFinance }">
                  {{ tutorFinance.fee }}
                </template>
                ...
                <template #actions="{ row: tutorFinance }">
                  <button type="button" aria-label="Update" class="btn btn-light-info me-2 p-2"
                    @click="openModal(tutorFinance.id)">
                    {{ t("global.show-details") }}
                  </button>
                  <button v-if="userType === 0" type="button" aria-label="Update" class="btn btn-light-success me-2 p-2"
                    @click="openUpdateModal(tutorFinance.id)">
                    {{ t("global.update") }}
                  </button>
                </template>
              </data-table>

              <div id="tutorTransferModal" class="modal fade" tabindex="-1" aria-labelledby="exampleModalLabel"
                aria-hidden="true">
                <div class="modal-dialog modal-lg">
                  <div class="modal-content">
                    <div class="modal-header">
                      <h5 id="exampleModalLabel" class="modal-title">
                        {{ t("global.tutor-finances-transfer") }}
                      </h5>
                      <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                      <div v-if="idCurrent !== null">
                        <template v-if="data.find((tutorFinance) => tutorFinance.id === idCurrent).method === 'paypal'">
                          <p>
                            {{ t("global.are-you-sure-to-transfer") }}
                            {{data.find((tutorFinance) => tutorFinance.id === idCurrent).amount}}
                            {{ t("global.to-this-tutor-paypal-account") }}:
                            {{data.find((tutorFinance) => tutorFinance.id === idCurrent).paypal_account}} ?
                          </p>
                        </template>
                        <button type="button" aria-label="Update" class="btn btn-light-success me-2 p-2" @click="
                          transferPayout(data.find((tutorFinance) => tutorFinance.id === idCurrent).id)
                          ">
                          {{ t("global.transfer") }}
                        </button>
                      </div>
              ...
              <div id="tutorFinanceModal" class="modal fade" tabindex="-1">
                <div class="modal-dialog modal-md modal-dialog-centered">
                  <div class="modal-content">

                    <div class="modal-header">
                      <h5 class="modal-title">
                        {{ t("global.tutor-finance-details") }}
                      </h5>
                      <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                    </div>

                    <div class="modal-body" v-if="currentFinance">

                      <!-- Tutor Header -->
                      <div class="d-flex align-items-center mb-4 border-bottom pb-3">
                        <img :src="currentFinance.tutor?.avatar" class="rounded-circle me-3" width="60" height="60"
                          alt="avatar" />
                        <div>
                          <h5 class="mb-1">{{ currentFinance.tutor?.full_name }}</h5>
                          <small class="text-muted">
                            {{ currentFinance.tutor?.email }}
                          </small>
                        </div>
                      </div>

                      <!-- Finance Info -->
                      <div class="row g-3 border-bottom mb-4 pb-3">

                        <div class="col-md-6">
                          <div class="p-3 bg-light rounded">
                            <strong>{{ t("global.total") }}</strong>
                            <div class="fs-5 mt-1">
                              {{ currentFinance.total }} $
                            </div>
                          </div>
                        </div>
                        ...
                        <div class="col-md-6">
                          <div class="p-3 bg-light rounded">
                            ...
                            <div class="fs-5 mt-1 text-success">
                              {{ currentFinance.fee }} $
                            </div>
                          </div>
                        </div>
                          ...
                          <div class="p-3 bg-light rounded">
                            ...
                            <div class="mt-2">
                              <span class="badge" :class="{
                                'bg-warning text-dark': currentFinance.status === 'pending',
                                'bg-success': currentFinance.status === 'transferred',
                                'bg-danger': currentFinance.status === 'rejected'
                              }">
                                {{ financeStatusLabel }}
                              </span>
                            </div>
                      ...
                      <!-- Info -->
                      <div class="row g-3" v-if="currentFinance.ref_type == 4">
                        ...
                        <div class="col-md-6">
                          <div class="p-3 bg-light rounded">
                            <strong>{{ t("global.student") }}</strong>
                            <div class="fs-5 mt-1">
                              {{ currentFinance.student?.full_name }}
                            </div>
                          </div>
                        </div>
                        ...
                        <div class="col-md-6">
                          <div class="p-3 bg-light rounded">
                            ...
                            <div class="fs-5 mt-1 text-success">
                              {{ currentFinance.date?.start_date_time }}
                            </div>
                          </div>
                        </div>
                        <div class="col-md-6">
                          <div class="p-3 bg-light rounded">
                            ...
                            <div class="fs-5 mt-1 text-success">
                              {{ currentFinance.date?.end_date_time }}
                            </div>
                          </div>
                        </div>
                      </div>
                      ...
                      <!-- Info -->
                      <div class="row g-3" v-if="currentFinance.ref_type == 1">
                        ...
                        <div class="col-md-6">
                          <div class="p-3 bg-light rounded">
                            <strong>{{ t("global.group_classes") }}</strong>
                            <div class="fs-5 mt-1">
                              {{ currentFinance.group_class?.name }}
                            </div>
                          </div>
                        </div>
                        ...
                        <div class="col-md-6">
                          <div class="p-3 bg-light rounded">
                            ...
                            <div class="fs-5 mt-1 text-success">
                              {{ currentFinance.group_class?.classes }}
                            </div>
                          </div>
                        </div>
                    ...
                    <div v-else class="modal-body text-center text-muted py-5">
                      No data available
                    </div>
              ...
              <div id="updateModal" class="modal fade" tabindex="-1" aria-labelledby="updateModalLabel"
                aria-hidden="true">
                <div class="modal-dialog modal-lg">
                  <div class="modal-content">
                      ...
                      <form @submit.prevent="handleUpdate">
                        ...
                        <div class="mb-3">
                          ...
                          <select id="updateStatus" v-model="updateStatus" class="form-select">
                            ...
                            <option value="pending">{{ t("global.review") }}</option>
                            <option value="transferred">{{ t("global.paid") }}</option>
                          </select>
                        </div>
                        <div class="mb-3">
                          ...
                          <textarea id="updateNotes" v-model="updateNotes" class="form-control"></textarea>
                        </div>
                        <!-- Update Button -->
...
import { defineComponent, onBeforeMount, onMounted, provide, ref, watchEffect, computed } from "vue"
  ...
  setup() {
    ...
    watchEffect(() => {
      if (idCurrent.value !== null) {
        const selectedtutorFinance = data.value.find((tutorFinance) => tutorFinance.id === idCurrent.value)
        if (selectedtutorFinance) {
          // Set the values based on the selected payout
          updateStatus.value = selectedtutorFinance.status
          updateNotes.value = selectedtutorFinance.note
        }
      }
    })
    ...
    const currentFinance = computed(() => {
      ...
      return data.value.find(item => item.id === idCurrent.value) || []
    })
    const financeStatusLabel = computed(() => {
      if (!currentFinance.value) return ''
      switch (currentFinance.value.status) {
        case 'pending':
    ...
    const financeRefTypeLabel = computed(() => {
      if (!currentFinance.value) return ''
      switch (currentFinance.value.ref_type) {
        case 1:
    ...
    const transferPayout = (id) => {
      ...
      if (payoutToTransfer) {
        axiosClient
          .post(`/tutor-finance/${id}/transfer`)
          .then((response) => {
            // Handle success
    ...
    const handleUpdate = () => {
      ...
      if (idCurrent.value !== null) {
        ...
        axiosClient
          .post(`/tutor-finance/update/${idCurrent.value}`, updatedPayout)
          .then((response) => {
            // Handle success, e.g., show a success message
            Swal.fire("Success", "Tutor finance updated successfully", "success");
