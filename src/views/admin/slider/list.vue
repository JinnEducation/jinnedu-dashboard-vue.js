...
<template>
  <toolbar :title="t('global.slider-management')" />
        ...
        <div class="card-header border-0 pt-6">
          <div class="card-title">
            <div class="d-flex align-items-center position-relative my-1">
              ...
              <label for="search-slider" class="sr-only">{{ t("global.search-slider") }}</label>
              <input id="search-slider" type="text" name="search-slider" :placeholder="t('global.search-slider')"
          ...
          <div class="card-toolbar">
            <div data-kt-content-table-toolbar="base" class="d-flex justify-content-end">
              <template v-if="abilities.create">
                <router-link :to="`/dashboard/slider/create`" class="btn btn-primary">
                  ...
                  {{ t("global.add-button") }} {{ t("global.slider") }}
                </router-link>
              </template>
            </div>
          </div>
        ...
        <template v-else-if="data.length === 0">
          <div class="card-body p-0">
            <div class="card-px text-center py-20 my-10">
              ...
              <template v-if="abilities.create">
                <router-link :to="`/dashboard/slider/create`" class="btn btn-primary er fs-6 px-8 py-4">
                  {{ t("global.add-button") }} {{ t("global.slider") }}
                </router-link>
              </template>
            </div>
            <div class="text-center px-5">
              <img src="@/assets/media/illustrations/welcome.png" :alt="`Add Our slider Illustration`"
        ...
        <template v-else>
          <div class="card-body py-4">
            <data-table :data="data" :header="header" :checkbox-enabled="true" checkbox-label="id"
                ...
                <div class="d-flex align-items-center">
                  ...
                  <div>
                    ...
                    <div class="text-muted small">
                      {{ typeof post.btn_name === 'object' ? post.btn_name[languageId] : post.btn_name }}
                    </div>
                  </div>
                </div>
              </template>
              ...
              <!-- العنوان الفرعي -->
              <template #sub_title="{ row: post }">
                <div class="text-muted small">{{ typeof post.sub_title === 'object' ? post.sub_title[languageId] : post.sub_title }}</div>
              </template>

              <!-- الأزرار -->
              <template v-if="abilities.edit || abilities.destroy" #actions="{ row: post }">
                <router-link v-if="abilities.edit" :to="`/dashboard/slider/update/${post.id}`" aria-label="Update"
                ...
                <button v-if="abilities.destroy" type="button" aria-label="Delete"
                  class="btn btn-icon btn-light-danger deletetooltip" @click="deleteslider(post.id)">
...
export default defineComponent({
  ...
  setup() {
    ...
    const deleteslider = (id) => {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          axiosClient.delete(`/slider/${id}`).then(() => {
            Swal.fire({
              icon: "success",
    ...
    const deleteFewOurslider = function deleteFewOurslider() {
    ...
    const registerslider = function registerslider(id) {
      loading.value = true
      axiosClient
        .post(`/slider/register-as-tutor/${id}`)
        .then(() => {
          getDataTableBodyRows()
          Swal.fire({
            icon: "success",
    ...
    onMounted(() => {
      initOurslider.value.splice(0, data.value.length, ...data.value)
    })
